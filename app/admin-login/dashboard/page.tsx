    ...
    const fetchDashboardData = async () => {
      try {
        ...
        if (analyticsData.realtime) {
          const activeCount = analyticsData.realtime.reduce(
            (acc: number, curr: any) => acc + curr.users,
            0,
          );
          const totalViews =
            analyticsData.topPages?.reduce((acc: number, curr: any) => acc + curr.views, 0) || 0;
          setStats((prev) => ({
            ...prev,
        ...
        if (inquiriesData.documents) {
          const latestActivities = inquiriesData.documents.slice(0, 5).map((doc: any) => ({
            label: `Inquiry: ${doc.name}`,
            description:
              doc.status === 'Queued'
                ? 'New inquiry received'
                : `Status updated to ${doc.status.replace('_', ' ')}`,
  ...
  return (
    <Link href={href} className="group relative block">
      <div className="h-full rounded-[2rem] border border-border bg-card p-8 transition-all duration-500 hover:border-border/80 hover:translate-y-[-4px] active:scale-[0.98] shadow-sm hover:shadow-lg">
        <div
          className={`mb-8 inline-flex p-4 rounded-2xl bg-muted transition-all duration-500 ${colorClasses}`}
        >
          {icon}
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-border/50 flex items-center justify-between">
          <span className="text-xs font-bold text-muted-foreground/60 uppercase tracking-widest group-hover:text-foreground">
            Open {title}
          </span>
...
function QuickActionLink({
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <span className="p-2 bg-muted rounded-lg group-hover:bg-primary-foreground/20 text-foreground group-hover:text-primary-foreground transition-colors">
          {icon}
        </span>
        <span className="font-bold text-sm uppercase tracking-tighter text-foreground group-hover:text-primary-foreground transition-colors">
          {label}
        </span>
      </div>
