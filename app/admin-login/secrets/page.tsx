...
export default function SecretsPage() {
  ...
  const fetchAuditLogs = async () => {
    ...
    try {
      const { jwt } = await account.createJWT();
      const response = await fetch(`/api/admin/doppler/audit?project=${selectedProject}`, {
        headers: { 'X-Appwrite-JWT': jwt },
      });
  ...
  const saveSecret = async () => {
    ...
    try {
      ...
      if (editingSecret && editingSecret.name !== name) {
        ...
        await fetch(
          `/api/admin/doppler/secrets?project=${selectedProject}&config=${selectedConfig}&name=${editingSecret.name}`,
          {
            method: 'DELETE',
            headers: { 'X-Appwrite-JWT': jwt },
          },
        ...
        <div className="flex gap-2">
          <Button
            ...
            onClick={() => {
              ...
              const content = secretsArray.map(([key, val]) => `${key}=${val.computed}`).join('\n');
          ...
          <Button onClick={syncToVercel} disabled={syncing || !selectedProject} variant="outline">
            {syncing ? (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1 space-y-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <Button variant="ghost" size="sm" onClick={() => toggleReveal(name)}>
      ...
      <Dialog open={bulkDialogOpen} onOpenChange={setBulkDialogOpen}>
        <DialogContent className="max-w-2xl h-[80vh] flex flex-col">
          ...
          <DialogFooter>
            ...
            <Button
              onClick={saveBulkSecrets}
              disabled={bulkSaving}
              className="bg-[#CC2224] hover:bg-black"
            >
              {bulkSaving ? (
