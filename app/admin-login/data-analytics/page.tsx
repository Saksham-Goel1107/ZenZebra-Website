...
export default function ZenZebraAnalyticsPage() {
  ...
  const ANALYTICS_COLLECTION_ID =
    process.env.NEXT_PUBLIC_APPWRITE_ANALYTICS_COLLECTION_ID || 'analytics_results';
  ...
  useEffect(() => {
    ...
    const filtered = analyses.filter((a) => {
      const matchesSearch =
        a.stockFileName.toLowerCase().includes(lower) ||
    ...
    try {
      ...
      const response = await fetch('/api/admin/analytics/trigger', {
        ...
        body: JSON.stringify({
          ...
          deadStockFileId,
          deadStockFileName: deadStockFile ? deadStockFile.name : null,
        }),
      });
      ...
      <div className="flex justify-between items-center print:hidden">
        <div>
          <h1 className="text-4xl font-black italic uppercase tracking-tighter">
            ZenZebra <span className="text-[#CC2224]">Analytics</span>
          </h1>
          <p className="text-muted-foreground">
            Advanced Intelligence: Sales, Stock, Customers & Dead Inventory
          </p>
        </div>
        ...
        <CardContent className="pt-6 grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
          <div className="space-y-2">
            <label className="font-bold text-sm flex items-center gap-2">
              <Package className="w-4 h-4 text-[#CC2224]" /> Stock Report *
            </label>
            <Input
              type="file"
              accept=".xlsx"
            ...
            />
            {stockFile && (
              <span className="text-xs text-green-600 font-bold">{stockFile.name}</span>
            )}
          </div>
          <div className="space-y-2">
            <label className="font-bold text-sm flex items-center gap-2">
              <ShoppingCart className="w-4 h-4 text-[#CC2224]" /> Sales Report *
            </label>
            <Input
              type="file"
              accept=".xlsx"
            ...
            />
            {salesFile && (
              <span className="text-xs text-green-600 font-bold">{salesFile.name}</span>
            )}
          </div>
          <div className="space-y-2">
            <label className="font-bold text-sm flex items-center gap-2">
              <Skull className="w-4 h-4" /> Dead Stock (Optional)
            </label>
            <Input
              type="file"
              accept=".xlsx"
              onChange={(e) => setDeadStockFile(e.target.files?.[0] || null)}
              disabled={analyzing}
            />
            {deadStockFile && (
              <span className="text-xs text-green-600 font-bold">{deadStockFile.name}</span>
            )}
          </div>
          <Button
            onClick={handleUpload}
            disabled={analyzing || !stockFile || !salesFile}
            className="bg-[#CC2224] hover:bg-black text-white font-bold w-full"
          >
            {analyzing ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="space-y-4 lg:col-span-1 border-r pr-4 h-screen sticky top-0 overflow-hidden flex flex-col print:hidden">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-bold text-lg flex items-center gap-2">
              <FileSpreadsheet className="w-5 h-5 text-[#CC2224]" /> History
            </h3>
            ...
            {filteredAnalyses.map((analysis) => (
              ...
              >
                <div className="flex justify-between text-xs mb-1">
                  <span
                    className={`px-2 py-0.5 rounded-full font-bold uppercase ${analysis.status === 'completed' ? 'bg-green-100 text-green-700' : analysis.status === 'failed' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}
                  >
                    {analysis.status}
                  </span>
        ...
        <div className="lg:col-span-3">
          {selectedAnalysis ? (
            ...
            >
              {(selectedAnalysis.status === 'completed' && analysisData) ||
              ...
              {analyzing ? (
                ...
                <div className="text-center space-y-4">
                  ...
                  <p className="text-zinc-500 max-w-sm mx-auto">
                    Upload new reports or select a past report from the sidebar.
                  </p>
                </div>
              )}
            </div>
