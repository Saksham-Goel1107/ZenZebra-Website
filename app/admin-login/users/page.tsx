  ...
  return (
    <div className="min-h-screen bg-background pb-20 p-4 md:p-8 space-y-10">
            ...
            <DialogContent className="bg-card border-border text-foreground sm:max-w-[425px] overflow-hidden">
              ...
              <DialogHeader>
                ...
                <DialogDescription className="text-muted-foreground">
                  Initial access requires a system-generated password. Operatives should update
                  their credentials upon first entry.
                </DialogDescription>
              </DialogHeader>
              <form onSubmit={handleCreateUser} className="space-y-5 mt-6">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-[9px] text-muted-foreground italic px-1">
                    Minimum 8 characters. Must contain varied complexity.
                  </p>
                </div>
                <Button
                  type="submit"
      ...
      <div className="flex flex-col md:flex-row items-center gap-4 bg-card p-4 rounded-2xl border border-border">
        ...
        <Button
          variant="outline"
          className="border-input bg-background hover:bg-accent text-foreground h-12"
        >
        ...
        <AnimatePresence mode="popLayout">
          {loading ? (
            ...
            >
              ...
              <h3 className="text-xl font-bold text-foreground uppercase italic tracking-wider">
                No matching operatives
              </h3>
              <p className="text-muted-foreground text-sm mt-2 max-w-xs">
                We couldn't find any team members matching your search criteria. Check spelling or
                clear filters.
              </p>
              <Button
                variant="link"
                ...
                <div className="relative rounded-[1.95rem] bg-card border border-border p-6 shadow-xl transition-all duration-300 group-hover:-translate-y-1">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        <div
                          className={`w-14 h-14 rounded-2xl flex items-center justify-center font-bold text-xl border shadow-lg overflow-hidden ${user.status ? 'bg-green-500/10 border-green-500/20 text-green-500' : 'bg-red-500/10 border-red-500/20 text-red-500'}`}
                        >
                          {user.prefs?.avatarId ? (
                            <img
                              src={storage
                                .getFilePreview(appwriteConfig.bucketId, user.prefs.avatarId)
                                .toString()}
                              alt={user.name}
                      ...
                      <DialogContent className="bg-card border-border text-foreground text-center">
                        <DialogHeader>
                          ...
                          <DialogDescription className="text-muted-foreground pt-2">
                            ...
                            <span className="text-foreground font-bold">{user.name}</span> from the
                            system. They will no longer be able to log in to the admin portal.
                          </DialogDescription>
                        </DialogHeader>
                        <DialogFooter className="mt-8 flex gap-3 sm:justify-center">
                          <Button
                            variant="ghost"
                            className="text-foreground hover:bg-accent border border-border flex-1"
                          >
                            Cancel
                          </Button>
                          <Button
                  ...
                  <div className="mt-6 pt-4 border-t border-border flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div
                        className={`px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider border ${user.emailVerification ? 'bg-blue-500/10 border-blue-500/20 text-blue-500' : 'bg-orange-500/10 border-orange-500/20 text-orange-500'}`}
                      >
