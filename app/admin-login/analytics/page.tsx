            ...
            {data?.error && (
                <div className="bg-destructive/10 border border-destructive/20 p-6 rounded-3xl flex items-start gap-4">
                    ...
                    <div>
                        ...
                        {data.details && <code className="block mt-3 bg-muted p-3 rounded-lg text-xs font-mono text-destructive break-all border border-destructive/10">{data.details}</code>}
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 rounded-[2.5rem] border border-border bg-card text-card-foreground p-8 shadow-sm relative overflow-hidden">
                    ...
                    <div className="flex items-center justify-between mb-8 relative z-10">
                        <div>
                            <h3 className="text-xl font-black text-foreground uppercase italic tracking-tighter">Traffic Growth</h3>
                            <p className="text-muted-foreground text-sm font-medium">Daily user trends over last 28 days</p>
                        </div>
                    </div>
                    <div className="h-[350px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={data?.daily || []}>
                                ...
                                <XAxis dataKey="date" tickFormatter={formatDate} axisLine={false} tickLine={false} tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 10 }} dy={10} minTickGap={30} />
                                ...
                                />
                                <Area type="monotone" dataKey="users" stroke="#CC2224" strokeWidth={3} fillOpacity={1} fill="url(#colorUsers)" />
                    ...
                    <div className="flex-1 rounded-[2.5rem] border border-border bg-card text-card-foreground p-8 shadow-sm relative overflow-hidden">
                        ...
                        <div className="space-y-4">
                            {(data?.realtime || []).slice(0, 5).map((user, i) => (
                                <div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-muted/50 border border-border/50">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-bold text-foreground capitalize">{user.city || 'Unknown City'}</p>
                                            <p className="text-xs text-muted-foreground">{user.country}</p>
                                        </div>
                                    </div>
                                    <span className="text-lg font-black text-foreground">{user.users} <span className="text-[10px] text-muted-foreground font-normal uppercase">Active</span></span>
                                </div>
                            ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="rounded-[2.5rem] border border-border bg-card text-card-foreground p-8 shadow-sm">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h3 className="text-xl font-black text-foreground uppercase italic tracking-tighter">Top Content</h3>
                    ...
                    <div className="space-y-4">
                        {(data?.topPages || []).map((page, i) => (
                            <div key={i} className="group p-4 rounded-2xl bg-muted/30 hover:bg-muted/60 border border-border/50 transition-all">
                                <div className="flex items-center justify-between mb-2">
                                    <h4 className="font-bold text-foreground text-sm truncate max-w-[70%]">{page.title}</h4>
                                    <span className="text-primary font-black text-lg">{page.views}</span>
                                </div>
                                <div className="flex items-center justify-between text-xs text-muted-foreground">
                                    <span className="bg-background px-2 py-1 rounded-md font-mono border border-border">{page.path}</span>
                ...
                <div className="flex flex-col gap-6">
                    <div className="flex-1 rounded-[2.5rem] border border-border bg-card text-card-foreground p-8 shadow-sm">
                        <h3 className="text-xl font-black text-foreground uppercase italic tracking-tighter mb-2">User Source</h3>
                        <p className="text-muted-foreground text-sm font-medium mb-6">Where users are coming from</p>

                        <div className="h-[250px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart layout="vertical" data={(data?.acquisition || []).slice(0, 5)} margin={{ left: 0, right: 30 }}>
                            ...
                            {(data?.acquisition || []).slice(0, 5).map((item, i) => (
                                <div key={i} className="flex items-center justify-between text-sm">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-foreground/80 capitalize">{item.source} <span className="text-muted-foreground">/ {item.medium}</span></span>
                                    </div>
                    ...
                    <div className="rounded-[2.5rem] border border-border bg-card text-card-foreground p-8 shadow-sm flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-muted-foreground text-sm">{data?.tech[0]?.browser || 'Unknown'}</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <span className="block text-2xl font-black text-foreground">{data?.tech[0]?.users || 0}</span>
                            <span className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">Users</span>
                        </div>
                    </div>
                </div>
            </div>
...
function StatCard({ title, value, subValue, icon, trend, pulse = false }: { title: string; value: string | number; subValue: string; icon: React.ReactNode; trend: string; pulse?: boolean }) {
    return (
        <div className="rounded-[2rem] border border-border bg-card text-card-foreground p-6 hover:border-primary/30 transition-all duration-500 group overflow-hidden relative shadow-sm hover:shadow-lg">
            <div className={`absolute top-0 right-0 w-32 h-32 bg-primary/5 blur-[50px] pointer-events-none group-hover:bg-primary/10 transition-all duration-700 ${pulse ? 'animate-pulse' : ''}`} />
            <div className="flex items-start justify-between mb-4 relative z-10">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground bg-muted px-2 py-1 rounded-lg">{trend}</span>
            </div>
